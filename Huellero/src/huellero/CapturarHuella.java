...
//EstadoHuellas();
    ...
    public void guardarHuella() {
        ...
        String sql=""
                + "SELECT dfi.huella  \n" +
                "	FROM sal_hc_movimiento_atencion mate,sal_hc_detalle_firma dfi \n" +
                "	WHERE mate.id = dfi.movimiento_atencion_id \n" +
                "		AND mate.cita_id IN (\n" +
                "			SELECT cit.id \n" +
                "			FROM sal_cita cit, con_tercero ter \n" +
                "			WHERE cit.tercero_id = ter.id \n" +
                "			AND cit.empresa_id ="+empresaID+" \n" +
        ...
        try 
        {
            ...
            if (rs.next()) {
                    ...
                    }else
                    {
                      ...
                      String sql2="	UPDATE sal_hc_detalle_firma dfi\n" +
                            "		SET huella =? \n" +
                            "	WHERE dfi.movimiento_atencion_id IN (\n" +
                            "		SELECT mate.id\n" +
                            "		FROM sal_hc_movimiento_atencion mate, sal_cita cit, con_tercero ter\n" +
                            "		WHERE mate.cita_id = cit.id AND cit.tercero_id = ter.id\n" +
                            "			AND cit.empresa_id = ter.empresa_id\n" +
                            "			AND ter.empresa_id = "+empresaID+"\n" +
                            "			AND ter.documento = '"+cedula+"'\n" +
                            "and cit.fecha_cita = DATE(now()))";
    ...
    public void identificarHuella() throws IOException {
        try {
            ...
            String identificarSQL="SELECT dfi.huella,nombre_completo(ter.id) as nombre_completo \n" +
            "	FROM sal_hc_movimiento_atencion mate, sal_hc_detalle_firma dfi, \n" +
            "	sal_cita cit, con_tercero ter\n" +
            "	WHERE mate.id = dfi.movimiento_atencion_id AND ter.empresa_id="+empresaID+" \n" +
            "and mate.cita_id=cit.id\n" +
            "and cit.tercero_id=ter.id AND cit.empresa_id=ter.empresa_id \n" +
            "	AND cit.fecha_cita = DATE(now())";
